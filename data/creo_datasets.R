...
df_ingresos_orig <- bind_rows(base_original, duplicados) %>%
  mutate(
...
df_ingresos_orig <- change_case(datos_sucios, "Ciudad Residencia")

# Ver resultado
df_ingresos_orig

# Guardar como .csv
write_csv(df_ingresos_orig, "data/base_personas_ingreso_anual_2024.csv")
